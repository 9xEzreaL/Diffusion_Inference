...
class Network(BaseNetwork):
    ...
    @torch.no_grad()# equal to p_sample_ddim
    def p_sample(self, y_t, t, clip_denoised=True, y_cond=None, guided_step=0, guided_mask=None, task='inpaint'):
        ...
        y_0_hat = self.predict_start_from_noise(y_t, t=t, noise=pred_noise)
        if guided_mask is not None and t[0] > guided_step:
            y_0_hat = y_cond * (1 - guided_mask) + y_0_hat * guided_mask
        # if clip_denoised:
        #     y_0_hat.clamp_(-1., 1.)
        direction_yt = (1. - alphas_prev - sigmas**2).sqrt() * pred_noise
        noise = sigmas * torch.randn_like(y_t) if any(t > 0) else torch.zeros_like(y_t)

        x_prev = alphas_prev.sqrt() * y_0_hat + direction_yt + noise#* noise

        return x_prev, y_0_hat
    ...
    def restoration(self, y_cond, y_t=None, y_0=None, mask=None, sample_num=8, guided_step=0, guided_mask=None,
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            y_t, pred_y_0 = self.p_sample(y_t, t, y_cond=y_cond, guided_step=guided_step, guided_mask=guided_mask, task=task)
            if mask is not None:
