...
class FeatureWiseAffine(nn.Module):
    ...
    def forward(self, x, noise_embed):
        ...
        if self.use_affine_level:
            gamma, beta = self.noise_func(noise_embed).view(batch, -1, 1, 1, 1).chunk(2, dim=1)
            x = (1 + gamma) * x + beta
        else:
...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        attn = torch.einsum("bnhwd, bncdhw -> bnhwd", query, key).contiguous() / math.sqrt(channel)
        attn = attn.view(batch, n_head, depth, height, width, -1)
        attn = torch.softmax(attn, dim=-1)
        attn = attn.view(batch, n_head, depth, height, width, depth, height, width)

        out = torch.einsum("bnhwd, bncdhw -> bnhwd", attn, value).contiguous()
...
class ResnetBlocWithAttn(nn.Module):
    def __init__(self, dim, dim_out, noise_level_emb_dim=None, norm_groups=32, dropout=0, with_attn=False):
        ...
        self.res_block = ResnetBlock(
            dim, dim_out, noise_level_emb_dim, dropout=dropout, norm_groups=norm_groups)
        if with_attn:
...
if __name__=="__main__":
    model = UNet()
    dummy_input = torch.rand((1, 6, 64, 64, 64))
    out = model(dummy_input, torch.Tensor([5]).long())
    print(out.shape)