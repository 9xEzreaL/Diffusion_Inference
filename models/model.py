...
class Palette(BaseModel):
    ...
    def set_input(self, data):
        ...
        self.cond_image = self.set_device(self.check_input_size(data.get('cond_image')))
        self.gt_image = self.set_device(self.check_input_size(data.get('gt_image')))
        self.mask = self.set_device(self.check_input_size(data.get('mask')))
        self.mask_image = self.check_input_size(data.get('mask_image'))
        self.path = data['path']
        self.batch_size = len(data['path'])


        if self.task in ['uncond_noisy_guided']:
            self.guided_mask = self.set_device(self.check_input_size(data.get('real_mask')))
        else:
            self.guided_mask = None

    def check_input_size(self, data):
    ...
    def get_current_visuals(self, phase='train'):
        dict = {
            ...
            'cond_image': (self.cond_image.detach()[:,0:1,::].float().cpu() + 1) / 2,
        }
    ...
    def train_step(self):
        ...
        for scheduler in self.schedulers:
            scheduler.step()
        for _, value in self.train_metrics.result().items():
            print(value)
    ...
    def test(self, phase_data, running_time=1, guided_step=0):
        self.netG.eval()
        ...
        with torch.no_grad():
            ...
            for key, value in self.get_current_visuals(phase='test').items():
                self.writer.add_images(key, value)
            self.writer.save_images(self.save_current_results(), running_time)
